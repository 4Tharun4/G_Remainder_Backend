...
const data_1 = require("../db/data"); // Import necessary database functions
const VerifyPasswordToken_1 = __importDefault(require("../helpers/VerifyPasswordToken"));
const updateUserInfo = async () => {
    try {
        const users = await (0, data_1.getdata)();
        for (const user of users) {
            const { userid, password } = user;
            // console.log(`Processing user: ${userid}`);
            const verifypassword = (0, VerifyPasswordToken_1.default)(password);
            ...
            // Update user data
            const updated = await (0, data_1.updateUserData)(userid, {
...
const scrapeTasks = async (userid, password) => {
